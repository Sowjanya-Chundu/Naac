...
<body>
    <div class="container">
        ...
        <form method="post">
            ...
            <input type="number" name="enrollmentyear" placeholder="Enrollment Year" required>
        ...
        if (request.getMethod().equalsIgnoreCase("post")) {
            ...
            try {
                ...
                String sqlInsert = "INSERT INTO studentsdata (Name, enrollmentyear, rollno) VALUES (?, ?, ?)";

                // Prepare the SQL statement for insertion
                stmtInsert = con.prepareStatement(sqlInsert);
                stmtInsert.setString(1, name);
                stmtInsert.setInt(2, enrollmentYear);
                stmtInsert.setString(3, rollNo);
                ...
                if (rowsInserted > 0) {
                    out.println("<div class='message'>Student information inserted successfully!</div>");

                    // Query to get updated total count
                    Statement stmtCount = con.createStatement();
                    rsCount = stmtCount.executeQuery("SELECT COUNT(*) AS total FROM studentsdata");

                    if (rsCount.next()) {
                        int totalCount = rsCount.getInt("total");
                        out.println("<div class='message'>Total students count updated successfully!</div>");
                        out.println("<div class='total-count'>Total students: " + totalCount + "</div>");
                    } else {
                        out.println("<div class='message'>Failed to retrieve total students count.</div>");
                    }
                }
                ...
                out.println("<div class='message'>Exception: " + e.getMessage() + "</div>");
                e.printStackTrace();
            } finally {
                // Close the resources
                try {
    ...
    <a href="<%=request.getContextPath()%>/AdminStudentFullTimeTeacher.html">Home</a>
</div>
</body>
</html>
