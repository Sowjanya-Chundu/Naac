...
try {
    ...
    out.println("<thead><tr><th>S.No</th><th>3rd Btech</th><th>Name of the Counseller</th><th>Mobile No</th><th>Actions</th></tr></thead>");
    out.println("<tbody>");
    while (result.next()) {
        try {
...
<div id="addModal" class="modal">
    <div class="modal-content">
        ...
        <form id="addForm" method="post" action="AdminStudTeacAddRecord3rd.jsp">
            <label for="addSno">S.No:</label>
            <input type="text" id="addSno" name="sno" required><br>
            <label for="addRegdno">3rd Btech:</label>
...
<div id="editModal" class="modal">
    <div class="modal-content">
        ...
        <h2>Edit Record</h2>
        <form id="editForm" method="post" action="AdminStudTeacEditRecord3rd.jsp">
            <input type="hidden" id="editId" name="sno">
            <label for="editSno">S.No:</label>
            <input type="text" id="editSno" name="sno" required readonly><br>
            <label for="editRegdno">3rd Btech:</label>
...
<script>
    function downloadData() {
        ...
        var opt = {
            margin:       1,
            filename:     '3rd_btech_councelling_list.pdf',
    ...
    function showEditModal(sno, regdno, name, mobile) {
        document.getElementById('editId').value = sno; // Hidden field for S.No (for internal use)
        document.getElementById('editSno').value = sno; // Read-only field for S.No
        document.getElementById('editRegdno').value = regdno || '';  // Handle null or undefined
        document.getElementById('editName').value = name || '';      // Handle null or undefined
        document.getElementById('editMobile').value = mobile || '';  // Handle null or undefined
    ...
    function deleteRow(sno) {
        if (confirm("Are you sure you want to delete this record?")) {
            ...
            xhr.open("GET", "AdminStudTeacDeleteRecord3rd.jsp?sno=" + sno, true);
