...
<%
    ...
    try {
        ...
        // Update SQL query
        String sql = "UPDATE project.councel3rdbtech SET `IIIB.TECH`=?, `Name of the Counselor`=?, `Counselor Mobile Number`=? WHERE `S.No`=?";
        ...
        sql = "SELECT * FROM project.councel3rdbtech";
        pst = con.prepareStatement(sql);
        ResultSet result = pst.executeQuery();

        out.println("<table>");
        out.println("<thead><tr><th>S.No</th><th>3rd Btech</th><th>Name of the Counselor</th><th>Mobile No</th></thead>");
        out.println("<tbody>");
        while (result.next()) {
            int dbSno = result.getInt("S.No");
            String updatedRegdno = result.getString("IIIB.TECH");
            String updatedName = result.getString("Name of the Counselor");
            String updatedMobile = result.getString("Counselor Mobile Number");
            out.println("<tr>");
