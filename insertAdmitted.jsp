...
<div class="container">
    ...
    <%  
        ...
        int seatsSanctioned = Integer.parseInt(request.getParameter("seatsSanctioned"));
        int studentsAdmitted = Integer.parseInt(request.getParameter("studentsAdmitted"));
        ...
        try {
            ...
            String sql = "INSERT INTO studentadmitted (`Programme Name`, `Programme Code`, `Number of seats sanctioned`, `Number of Students admitted`) " +
                         "VALUES (?, ?, ?, ?)";
            stmt = con.prepareStatement(sql);
            stmt.setString(1, programmeName);
            stmt.setString(2, programmeCode);
            stmt.setInt(3, seatsSanctioned);
            stmt.setInt(4, studentsAdmitted);

            int rowsInserted = stmt.executeUpdate();
            if (rowsInserted > 0) {
    %>
                <div class="message">
                    <p>New admission details inserted successfully!</p>
                    <p>Programme Name: <%= programmeName %></p>
                    <p>Programme Code: <%= programmeCode %></p>
                    <p>Number of Seats Sanctioned: <%= seatsSanctioned %></p>
                    <p>Number of Students Admitted: <%= studentsAdmitted %></p>
                </div>
    <%
            } else {
    %>
                <div class="message">
                    <p>Failed to insert admission details.</p>
                </div>
    <%
            }
