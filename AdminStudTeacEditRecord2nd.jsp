...
<%
    ...
    try {
        ...
        // Update SQL query
        String sql = "UPDATE project.councel2ndbtech SET `II  B.TECH`=?, `Name of the Counselor`=?, `Counselor Mobile Number`=? WHERE `S.No`=?";
        ...
        sql = "SELECT * FROM project.councel2ndbtech";
        pst = con.prepareStatement(sql);
        ResultSet result = pst.executeQuery();

        out.println("<table>");
        out.println("<thead><tr><th>S.No</th><th>2nd Btech</th><th>Name of the Counselor</th><th>Mobile No</th></thead>");
        out.println("<tbody>");
        while (result.next()) {
            int dbSno = result.getInt("S.No");
            String updatedRegdno = result.getString("II  B.TECH");
